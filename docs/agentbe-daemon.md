...
For the full behavioral specification (endpoints, authentication, transport details, Docker image, shutdown semantics, etc.), see [opensdd/daemon.md](../packages/agent-backend/opensdd/daemon.md). This document covers usage and configuration.
...
For detailed security documentation, see [docs/security.md](security.md). For implementation-level security contracts (authentication, path jailing, command safety patterns), see the [daemon spec](../packages/agent-backend/opensdd/daemon.md#security-considerations) and [safety spec](../packages/agent-backend/opensdd/safety.md).
