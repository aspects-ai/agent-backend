...
**Done — agent-sandbox integration (2026-07-28), verified on kind + Calico.** Warm pools via `SandboxWarmPool`/`SandboxClaim`, so a session's first query no longer waits on pod scheduling.
- **Measured: `open_session` 381ms, first command complete at 401ms** — against ~8–9s for the raw-pod provider on an already-warm local node (and 30–60s on a real cluster under scheduling pressure). This is TTFT, not amortized cost: the wait lands on the user's first query.
- **The credential constraint is real, and the controller enforces it.** `SandboxClaim.spec.env` works (requires `envVarsInjectionPolicy: Allowed`, default `Disallowed`) — but setting it makes the controller log *"Bypassing warm pool adoption because custom configuration is provided"* and build a **fresh** pod. Measured: claim with no env adopts an already-running pod in 0.2s; claim with env creates a new one. **Env injection and warm start are mutually exclusive.** So each pooled pod mints its own token at startup and serves it on :3002; the room fetches it at claim time. Needs no agent-backend change — a `command` override using bash + python3, both already in the daemon image.
- **Atomic allocation comes free**, which is the part worth *not* hand-rolling: two concurrent claims can never bind the same sandbox.
- **`shutdownPolicy: Delete`** (their default is `Retain`) so sandboxes are destroyed after use; `AGENTBE_SANDBOX_REUSE=1` opts into recycling for non-sensitive workloads, with the caveat documented at the call site — a recycled sandbox carries the previous session's files *and* its still-valid token.
- **NetworkPolicy: `networkPolicyManagement: Unmanaged` + our own.** Their managed policy admits ingress only from `app: sandbox-router` in `agent-sandbox-system`, **a component the release bundle does not deploy** — so the room could never reach a sandbox. Ours grants room→sandbox on 3001/3002 only and keeps their egress denials (all private ranges plus `169.254.0.0/16`, which blocks cloud metadata — better than what I had planned).
- **Enforcement verified, not assumed.** kind's default kindnet does **not** enforce NetworkPolicy, so the cluster was rebuilt with `disableDefaultCNI` + Calico. From inside a live sandbox: reaching another sandbox's token endpoint → **BLOCKED**; reaching the kube-apiserver → **BLOCKED**.
- Opt-in only (`AGENTBE_SANDBOX=agent-sandbox`) — it needs CRDs and a pool installed, so auto-detection must not assume them. The raw-pod `K8sWorkspaceProvider` remains the zero-install default.
- **Consolidated (2026-07-28):** both k8s providers now share `lib/k8s-api.ts`; `workspace-k8s.ts` went 300 → 191 lines. Coverage before the refactor was thinner than assumed — a single unit test asserting the constructor throws without a token, and the e2e harness had stopped exercising the raw-pod path entirely once both rooms moved to agent-sandbox. So the example deploy now runs **one room on each provider** (`acme` = agent-sandbox/warm pool, `globex` = raw pods), keeping both exercised. Verified post-refactor: `acme` claimed `agentbe-pool-*` in 454ms, `globex` created `agentbe-sandbox-*` in 1624ms; both read their own document, committed, and left nothing behind.
